...
@section('info')
    <title>{{$field->art_title}} - {{Config::get('web.website_title')}}</title>
...
@section('content')
    <article class="blogs">
        ...
        <div class="index_about">
            ...
            <div class="ad"> </div>
            <div class="nextinfo">
                <p>上一篇：<a href="{{url('news/'.$article['pre']->art_id)}}">{{$article['pre']->art_title}}</a></p>
                <p>下一篇：<a href="{{url('news/'.$article['next']->art_id)}}">{{$article['next']->art_title}}</a></p>
            </div>
            <div class="otherlink">
                <h2>相关文章</h2>
                <ul>
